...
def sdk_tarball():
    ...
    with tarfile.TarFile.open(f"tarball/MaaDeps-{tarball_triplet}-devel.tar.xz", 'w:xz') as sdktar:
        ...
        if (vcpkg.triplet.removeprefix('maa-').startswith('arm64-')):
            sdktar.add(f"./vcpkg/installed/{vcpkg.triplet.removeprefix('maa-').replace('arm64', 'x64')}/tools", filter=sdk_filter)
